...
func (b *PaginatorBuilder) fromMap(data map[string]any) *PaginatorBuilder {
	...
	// Handle notin (also "not_in" for struct compatibility)
	for _, key := range []string{"notin", "not_in"} {
		if notIn, ok := data[key]; ok {
	...
	// Handle gteor (also "gte_or" for struct compatibility)
	for _, key := range []string{"gteor", "gte_or"} {
		if gteOr, ok := data[key]; ok {
	...
	// Handle gtor (also "gt_or" for struct compatibility)
	for _, key := range []string{"gtor", "gt_or"} {
		if gtOr, ok := data[key]; ok {
	...
	// Handle lteor (also "lte_or" for struct compatibility)
	for _, key := range []string{"lteor", "lte_or"} {
		if lteOr, ok := data[key]; ok {
	...
	// Handle ltor (also "lt_or" for struct compatibility)
	for _, key := range []string{"ltor", "lt_or"} {
		if ltOr, ok := data[key]; ok {
	...
	// Handle inor (also "in_or" for struct compatibility)
	for _, key := range []string{"inor", "in_or"} {
		if inOr, ok := data[key]; ok {
			if m, ok := inOr.(map[string]any); ok {
				for k, v := range m {
					b.InOr(k, toInterfaceSlice(v)...)
				}
			} else if mSlice, ok := inOr.(map[string][]any); ok {
	...
	// Handle notinor (also "not_in_or" for struct compatibility)
	for _, key := range []string{"notinor", "not_in_or"} {
		if notInOr, ok := data[key]; ok {
			if m, ok := notInOr.(map[string]any); ok {
				for k, v := range m {
					b.NotInOr(k, toInterfaceSlice(v)...)
				}
			} else if mSlice, ok := notInOr.(map[string][]any); ok {
				for k, v := range mSlice {
					b.NotInOr(k, v...)
				}
			}
	...
	// Handle vacuum
	for _, key := range []string{"vacuum"} {
		if vacuum, ok := data[key]; ok {
			if v, ok := vacuum.(bool); ok && v {
				b.WithVacuum()
			}
			break
		}
	...
	// Handle no_offset / nooffset
	for _, key := range []string{"no_offset", "nooffset"} {
		if noOffset, ok := data[key]; ok {
			if v, ok := noOffset.(bool); ok && v {
				b.WithoutOffset()
			}
			break
		}
...
// Useful in handlers to pass to NewPage after the repository returns data.
func (b *PaginatorBuilder) CurrentPage() int {
	return b.params.Page
}

// CurrentLimit returns the current items-per-page set in the builder.
// Useful in handlers to pass to NewPage after the repository returns data.
func (b *PaginatorBuilder) CurrentLimit() int {
	return b.params.ItemsPerPage
}
